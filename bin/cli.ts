...
cmdCsv.addCommand(cli.run(program, pkg).name("export")
    ...
    .option('--folder_granular <int>', `folder granular (default: ${DEFAULT_FOLDER_GRANULAR})`, String(DEFAULT_FOLDER_GRANULAR))
    .option('--file_granular <int>', `file granular (default: ${DEFAULT_FILE_GRANULAR})`, String(DEFAULT_FILE_GRANULAR))
    .option('--collect-default-metrics <boolean>', "Collect default metrics", DEFAULT_COLLECT_DEFAULT_METRICS)
